...
<div class="row">
  ...
  <div class="col-sm-8"><h1>Estado de Pago</h1</div>
</div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <form class="w3-container" action="/Estado_Pago/delete" method="POST" onKeypress="if(event.keyCode == 13) event.returnValue = false;">
              {{ csrf_field() }}
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><h1>Crear Estado de Pago</h1></h4>
      </div>
      <div class="modal-body">
        <form class="form-horizontal" role="form" onKeypress="if(event.keyCode == 13) event.returnValue = false;">
          ...
          <div class="form-group row add">
            ...
            <div class="col-sm-10">
              <input type="text" class="form-control" id="cestado" name="cestado" onKeyUp="javascript:this.value = this.value.replace(/,/,'');document.getElementById(this.id).value=document.getElementById(this.id).value.toUpperCase()" 
...
<div class="modal fade" id="modificar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title"><h1>Modificar Estado de Pago</h1></h4>
      </div>
      <div class="modal-body">
        <form class="form-horizontal" role="form" onKeypress="if(event.keyCode == 13) event.returnValue = false;">
           <p class="error text-center alert alert-danger hidden"></p>
...
$("#tnombre").on('keyup', function(){
 ...
 $(document).on('click','.create-modal', function() {
  ...
  $("#add").click(function() {
    $.ajax({
      type: 'POST',
      url:'{{ url('addestadopago')}}',
      data: {
        '_token': $('input[name=_token]').val(),
        'cestado': $('input[name=cestado]').val()
      },
      success: function(data) {
                if ((data.errors)){
             $('.error').removeClass('hidden');
                    $('.error').text(data.errors.cestado);
