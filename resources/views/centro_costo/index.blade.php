...
<div class="row">
  ...
  <div class="col-sm-8"><h1>Centro de Costo</h1></div>
</div>
 

<button class="glyphicon glyphicon-plus" data-toggle="modal" data-target="#create">Agregar Centro</button>
<br>
<br>
<br>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <form class="w3-container" action="/Centro/delete" method="POST" onKeypress="if(event.keyCode == 13) event.returnValue = false;">
              {{ csrf_field() }}
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel"><i class="glyphicon glyphicon-erase"></i>Estas seguro de eliminar 
          

          <input  border="0" onKeyUp="javascript:this.value = this.value.replace(/,/,'');document.getElementById(this.id).value=document.getElementById(this.id).value.toUpperCase()"  id="enombre" name="enombre" type="text" value="" readonly=""> </h4>
      ...
      <div class="modal-body">
        <form class="form-horizontal" role="form" onKeypress="if(event.keyCode == 13) event.returnValue = false;">
          ...
          <div class="form-group row add">
            <label class="control-label col-sm-2" for="cnombre_cac">Nombre :</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" id="cnombre_cc" name="cnombre_cc" onKeyUp="javascript:this.value = this.value.replace(/,/,'');document.getElementById(this.id).value=document.getElementById(this.id).value.toUpperCase()" 
  ...
  $("#add").click(function() {
    $.ajax({
      ...
      data: {
        ...
        'cnombre_cc': $('input[name=cnombre_cc]').val()
      },
      success: function(data) {
                if ((data.errors)){
                    $('.error').removeClass('hidden');
                    $('.error').text(data.errors.cnombre_cc);
